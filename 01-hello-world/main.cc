...
int main(){
    ...
    FunctionType *putsFunType = FunctionType::get(irBuilder.getInt32Ty(), {irBuilder.getPtrTy()}, false);
    Function *putsFunc = Function::Create(putsFunType, GlobalValue::LinkageTypes::ExternalLinkage, "puts", m);


    llvm::Constant *c = ConstantDataArray::getString(context, "helloworld");
    GlobalVariable *gvar = new GlobalVariable(m, c->getType(), true, GlobalValue::LinkageTypes::PrivateLinkage, c, "gstr");
